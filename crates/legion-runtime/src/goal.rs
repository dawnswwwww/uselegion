//! Session goals: durable per-session objectives managed via `/goal` and the
//! model-facing `create_goal`/`get_goal`/`update_goal` tools.
//!
...
//! as a compact user-role context line on every turn, and the turn-end goal
//! gate (`crate::goal_gate`) keeps the run going while the goal is active.
...
pub enum GoalStatus {
    ...
    Blocked,
    /// Reserved for a future budget stop state.
    BudgetLimited,
...
impl GoalStore {
    ...
    pub async fn save(&self, session_key: &str, goal: &Goal) -> Result<(), GoalError> {
        ...
        legion_core::fs::atomic_write_async(&path, text.as_bytes()).await?;
        Ok(())
    }
    ...
    fn path_for(&self, session_key: &str) -> Result<PathBuf, GoalError> {
        let parts = legion_plugin_sdk::session_key::parse_session_key(session_key)
            .ok_or(GoalError::InvalidSessionKey)?;
        if !legion_plugin_sdk::session_key::is_safe_segment(&parts.agent_id)
            || !legion_plugin_sdk::session_key::is_safe_segment(&parts.peer_id)
        {
        ...
        Ok(self
            ...
            .join(&parts.agent_id)
            .join("goals")
            .join(format!("{}.json", parts.peer_id)))
    }
}
...
mod tests {
    ...
    #[test]
    fn store_default_uses_home_dot_legion() {
        let store = GoalStore::default();
        let expected = dirs::home_dir()
            .map(|h| h.join(".legion"))
            .unwrap_or_else(|| PathBuf::from(".legion"));
        let resolved = store
            .path_for("agent:main:dm:cli:default:direct:peer-1")
            .unwrap();
